...
export default function EtapaDecisiones({
  ...
  const secciones = [
    { id: "ingresos", nombre: "Ingresos", pendientes: sinDecidir.length },
    { id: "deducciones", nombre: "Deducciones", pendientes: (peticiones ?? []).length },
    { id: "patrimonio", nombre: "Patrimonio", pendientes: faltaPatrimonio ? 1 : 0 },
    { id: "alertas", nombre: "Alertas", pendientes: porConfirmar.length },
    { id: "hecho", nombre: "Ya decidido", pendientes: 0 },
  ];
  // SE ABRE DONDE HAY TRABAJO, no siempre en la primera. Abrir en "Ingresos" con los renglones ya
  // resueltos obligaba a buscar a mano cual de las cuatro tenia algo pendiente.
  const [seccion, setSeccion] = useState(
    () => (secciones.find((s) => s.pendientes) ?? secciones[0]).id,
  );
  ...
  return (
    <section className="etapa-cuerpo">
      ...
      {/* CUATRO ASUNTOS, NO UN SOLO SCROLL. Antes los renglones del cruce, los certificados por
          pedir, el patrimonio y las alertas iban uno detras de otro en la misma columna: para
          contestar si el cliente tiene carro habia que pasar por dieciocho renglones de ingresos.
          Son cosas de naturaleza distinta y se trabajan en cualquier orden, o sea que son PARES,
          que es exactamente el caso en el que una pestaña es lo correcto. (La etapa completa no lo
          es, y por eso arriba sigue siendo un progreso y no unas pestañas.) */}
      <div className="secciones" role="tablist">
        {secciones.map((sec) => (
          <button
            key={sec.id}
            role="tab"
            aria-selected={seccion === sec.id}
            className={seccion === sec.id ? "seccion seccion-activa" : "seccion"}
            onClick={() => setSeccion(sec.id)}
          >
            {sec.nombre}
            {/* El numero es lo que falta, no lo que hay: un contador necesita saber donde queda
                trabajo, no cuantas filas existen. Sin pendientes no se pinta nada, porque un cero
                en un contador se lee como un aviso. */}
            {sec.pendientes ? <span className="seccion-cuenta">{sec.pendientes}</span> : null}
          </button>
        ))}
      </div>

      {seccion === "ingresos" ? (
        sinDecidir.length || partidas.length ? (
          <Conciliacion caseId={caseId} conciliacion={conciliacion} onCambio={onCambio} />
        ) : (
          <p className="estado">No hay renglones que decidir.</p>
        )
      ) : null}
      ...
      {seccion === "deducciones" ? (
        (peticiones ?? []).length ? (
          ...
          <p className="estado">No falta ningún certificado por pedir.</p>
        )
      ) : null}
      ...
      {seccion === "alertas" ? (
        porConfirmar.length ? (
          <Pendientes caso={caso} onCambio={onCambio} />
        ) : (
          <p className="estado">No hay nada por confirmar.</p>
        )
      ) : null}
  ...
  return (
    <div className="decision">
      ...
      <p className="decision-pregunta">
        {preguntaDelRenglon({ porClasificar, ajena, hayDocumento: Boolean(doc), profunda })}
      </p>
      ...
      <div className="decision-cifras">
        ...
        {doc ? (
          <p>
            <span className="decision-quien">
              {profunda ? "El documento dice" : "Tu documento dice"}
            </span>
      ...
      {ajena ? (
        <p className="decision-nota">
          El tercero lo reportó a nombre de <strong>{partida.reportado_a}</strong>,
          {profunda ? " no al del cliente." : " no al tuyo."}
        </p>
      ) : null}
...
function Opcion({ caseId, partida, decision, motivos, ajena, profunda, abierta, onAbrir, onListo }) {
  ...
  // TRES BOTONES PEGADOS Y SIN JERARQUIA no son tres opciones: son un muro. "No es del cliente" y
  // "Lo pongo yo en el 210" hacen cosas opuestas —una saca la plata de la declaracion, la otra la
  // deja adentro pero fuera del calculo— y se veian identicos, uno encima del otro. Cada salida
  // dice ahora QUE HACE debajo de su nombre, que es lo unico que permite elegir entre dos cosas
  // que suenan parecido.
  const cuerpo = (
    <>
      <span className="salida-nombre">
        {esDelTitular ? <X size={14} /> : null}
        {resolver.running ? "Guardando…" : texto}
      </span>
      {QUE_HACE[decision] ? <span className="salida-que">{QUE_HACE[decision]}</span> : null}
    </>
  );

  if (directa) {
    return (
      <button className="salida" disabled={resolver.running} onClick={() => aplicar(motivos[0])}>
        {cuerpo}
      </button>
    );
  }
  ...
  return (
    <div className="opcion">
      <button
        className={`salida ${abierta ? "salida-abierta" : ""}`}
        onClick={onAbrir}
